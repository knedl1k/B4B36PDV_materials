...
Tento text vznikl jako snaha všechny důležité informace sjednotit na jednom místě, aby studenti předmětu měli v rukách
ucelený text, který je provede nejen motivacemi a úplnými základy použitých metod a strategií, ale dá jim především
praktické znalosti.

Přese vše je nutné dodat, že je text především cílen pro studující, kteří již absolvovali předměty Architektura
počítačů, Operační systémy a Algoritmizace. Bez těchto znalostí je stále možné náš text pochopit, velmi ale doporučujeme
prerekvzity doplnit.
...
#todo([Errata / celé PDF budeme udržovat na stránce])
